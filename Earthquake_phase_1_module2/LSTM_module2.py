from Earthquake_phase_1_module2 import EarthQuakePhase1
import numpy as np
import time
import random as rd
import pandas as pd
...
class LSTM(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.weights_matrix[1][0]=self.Weights_forget_1; self.weights_matrix[1][1]=self.Weights_forget_2; self.weights_matrix[1][2]=self.forget_gate_bias;
        self.weights_matrix[2][0]=self.Weights_output_1; self.weights_matrix[2][1]=self.Weights_output_2; self.weights_matrix[2][2]=self.output_gate_bias;
        ...
        self.delta_gate_list=np.zeros((3,1))
...
class LSTMNetwork():
        ...
        for cntr in range(5000):#35
            ...
            while listPtr<len(self.dataset)-1:
                ...
                for j in range(self.lstmCellsCnt):
                    ...
                    temp_previous_output,temp_previous_cell_state=self.forwardPass(lstmCell,cellDataSet[0])    #forward pass
    ...
    def updateWeights(self,lstmCellObjList):
        total_delta_W=np.zeros((3,1))
        total_delta_U=np.zeros((3,1))
        total_delta_b=np.zeros((3,1))
    ...
    def backPropagate(self,lstmCellObjList,cellLocation):        
        for cellLoc in range(len(lstmCellObjList)-1,-1,-1):     #back propagate the cell just forward propagated.
            ...
            lstmCellObj.delta_input_A_gate=np.dot(lstmCellObj.delta_cell_state,np.dot((np.array(1)-lstmCellObj.forget_gate),(np.array(1)-np.square(lstmCellObj.input_A_gate))))
        ...
        #self.weights_matrix[3][0]=self.Weights_output_1; self.weights_matrix[3][1]=self.Weights_output_2; self.weights_matrix[3][2]=self.output_gate_bias;
            ...
            lstmCellObj.delta_gate_list[1][0]=lstmCellObj.delta_forget_gate
            lstmCellObj.delta_gate_list[2][0]=lstmCellObj.delta_output_gate
            
            if cellLoc>0:
                weight_U_matrix=np.zeros((1,3))
                ...
                weight_U_matrix[0][1]=lstmCellObj.weights_matrix[1][1]  #lstmCellObj.Weights_forget_2
                weight_U_matrix[0][2]=lstmCellObj.weights_matrix[2][1]  #lstmCellObj.Weights_output_2
    ...
    def forwardPass(self,lstmCell,current_input):
        ...
        #input Activation gate computation
        ...
        lstmCell.cell_state=np.array((np.dot(lstmCell.forget_gate,lstmCell.cell_state_previous))+(np.dot((np.array(1) - lstmCell.forget_gate),lstmCell.input_A_gate)))
    ...
    def predict(self,current_input,test_data):
        ...
        while(slidingPtr<len(test_data)):
            ...
            for cnt in range(len(self.lstmCellObjList)):
                ...
                lstmCell.input_matrix_X[0]=inputList[0] #current_input
                lstmCell.input_matrix_U[0]=lstmCell.previous_output
                lstmCell.input_matrix_b[0]=1
            ...
            print('input:',inputList[0] ,' total prediction:',pd.to_datetime(pred_val*self.global_date_max),' pred:',pred_val,' threshold:',threshold,' length:',len(test_data))
...
global_date_max=1
lstmNet=LSTMNetwork(3,data,global_date_max) #25
...
#plt.plot(list(test_data),label='Test Data')
